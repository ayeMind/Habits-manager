...
const Store = () => {
  ...
  const handleBuyTheme = () => {
    ...
    if (savedThemes.length === 6) {
      ...
      setModalDescription(
        `Вы успешно купили новую тему! Вам выпала тема: ${theme}!`
      );
    }
    open();
  };
  ...
  const handleBuyImport = () => {
    ...
    if (templateImportIsOpen) {
      ...
      setModalDescription(
        "У вас недостаточно монет для покупки временного импорта!"
      );
    } else {
      openTemplateImport();
      setModalDescription(
        "Покупка прошла успешно! Воспользоваться временным импортом можно в настройках"
      );
    }
    open();
  };
  ...
  return (
    <PageLayout title="Магазин" defaultTab="store">
      ...
      <SimpleGrid mt={30} cols={{ base: 1, xs: 2, lg: 3, xl: 4, xxxl: 6 }}>
        <StoreCard
          title="Покупка темы"
          description="Выпавшие темы не будут повторятся"
          price={150}
          image={themeImage}
          onClick={handleBuyTheme}
        />
        <StoreCard
          title="Покука временного импорта"
          description="С этим вы сможете заменить свои ежедневные привычки на привычки вашего друга на один день"
          price={350}
          image={templateImportImage}
          onClick={handleBuyImport}
        />
      </SimpleGrid>
    </PageLayout>
  );
